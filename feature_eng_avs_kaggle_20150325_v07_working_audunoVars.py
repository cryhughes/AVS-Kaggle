	...
	for e, line in enumerate( gzip.open(gz_trans)):
		...
		if e > 0:
			...
			if last_id != row[0] and e != 1: #write features when we get to a new shopper id
				...
				features['company_share_wallet'] = zidz( features['company_total_amount'], features['company_total_quantity']) 
				#category
				features['category_buy_true'] = 1 if features['category_total_quantity'] > 0 else 0
				features['company_share_wallet'] = zidz( features['category_total_amount'], features['category_total_quantity']) 
				#brand
				features['company_buy_true'] = 1 if features['brand_total_quantity'] > 0 else 0
				features['company_share_wallet'] = zidz( features['brand_total_amount'], features['brand_total_quantity']) 
				#write out to file
