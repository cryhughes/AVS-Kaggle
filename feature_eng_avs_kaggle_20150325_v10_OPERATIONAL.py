...
feature_list = [\
				...
				"purchases_amount_30", "purchases_amount_60", "purchases_amount_90", \
				#company
				...
				"category_quantity_30", "category_quantity_60", "category_quantity_90", \
				"category_amount_30", "category_amount_60", "category_amount_90", \
				#brand
...
def diff_days(s1,s2):
	...
	s2 = s2.replace('"','')
	b = parse(s2)
	#date_format = "%Y-%m-%d"	#print type(s1) #type is string
	...
	for e, line in enumerate( gzip.open(gz_trans)):
		...
		if e > 0:
			...
			if row[0] in ids: 
				...
				cust_hist = ids[row[0]]
				#get timeVar array
				date_diff_days = diff_days(row[6],cust_hist[6+test_set_data_adj])
				if not testset_true:
					if cust_hist[5] =="t":
				...
				#purchase timeVars
				features['purchases_quantity_30'] += float( row[9] ) if date_diff_days < 30 else 0
				features['purchases_quantity_60'] += float( row[9] ) if date_diff_days < 60 else 0
				features['purchases_quantity_90'] += float( row[9] ) if date_diff_days < 90 else 0
				features['purchases_amount_30'] += float( row[10] ) if date_diff_days < 30 else 0
				features['purchases_amount_60'] += float( row[10] ) if date_diff_days < 60 else 0
				features['purchases_amount_90'] += float( row[10] ) if date_diff_days < 90 else 0
				#company		
				if offers[ cust_hist[2]] [3] == row[4]:
					features['company_total_quantity'] += float( row[9] )
					features['company_total_amount'] += float( row[10] )
					#company timeVars
					features['company_quantity_30'] += float( row[9] ) if date_diff_days < 30 else 0 
					features['company_quantity_60'] += float( row[9] ) if date_diff_days < 60 else 0
					features['company_quantity_90'] += float( row[9] ) if date_diff_days < 90 else 0
					features['company_amount_30'] += float( row[10] ) if date_diff_days < 30 else 0
					features['company_amount_60'] += float( row[10] ) if date_diff_days < 60 else 0
					features['company_amount_90'] += float( row[10] ) if date_diff_days < 90 else 0
				#category 
				if offers[ cust_hist[2] ][1] == row[3]:
					features['category_total_quantity'] += float( row[9] )
					features['category_total_amount'] += float( row[10] )
					#category timeVars
					features['category_quantity_30'] += float( row[9] ) if date_diff_days < 30 else 0 
					features['category_quantity_60'] += float( row[9] ) if date_diff_days < 60 else 0
					features['category_quantity_90'] += float( row[9] ) if date_diff_days < 90 else 0
					features['category_amount_30'] += float( row[10] ) if date_diff_days < 30 else 0
					features['category_amount_60'] += float( row[10] ) if date_diff_days < 60 else 0
					features['category_amount_90'] += float( row[10] ) if date_diff_days < 90 else 0
				#brand
				if offers[ cust_hist[2] ][5] == row[5]:
					features['brand_total_quantity'] += float( row[9] )
					features['brand_total_amount'] += float( row[10] )
					#brand timeVars
					features['brand_quantity_30'] += float( row[9] ) if date_diff_days < 30 else 0 
					features['brand_quantity_60'] += float( row[9] ) if date_diff_days < 60 else 0
					features['brand_quantity_90'] += float( row[9] ) if date_diff_days < 90 else 0
					features['brand_amount_30'] += float( row[10] ) if date_diff_days < 30 else 0
					features['brand_amount_60'] += float( row[10] ) if date_diff_days < 60 else 0
					features['brand_amount_90'] += float( row[10] ) if date_diff_days < 90 else 0
