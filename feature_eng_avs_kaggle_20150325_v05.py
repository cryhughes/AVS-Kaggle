...
def diff_days(s1,s2):
	...
	delta = b - a
	return delta.days
#helper function for feature creation: core_features
def write_basic_features( id ):
...
def generate_features( trans_file , out_file ):
	...
	for e, line in enumerate( gzip.open(gz_trans)):
		...
		if e > 0:
			...
			if last_id != row[0] and e != 1: #write features when we get to a new shopper id
				...
				if last_id in ids:
					outline += " "+ids[last_id][3] #chain
					outline += " "+ids[last_id][1] #market		
				outline += "\n"
		    ...
		    #    print 'Error at e', e, row[0]
		        #break			last_id = row[0] #set cust_id to row, for comparing to next line
	...
	print "completed loop for unseen_ids... seen ids: " + str(len(seen_ids)) + " unseen_ids: " +str(len(unseen_ids)) 
#actions
